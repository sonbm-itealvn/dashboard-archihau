...
<template>
  <section class="card page">
    <header class="page__header">
      <div>
        <p class="eyebrow">Thẻ nội dung</p>
        <h2>Quản lý thẻ</h2>
        <p>Gắn thẻ cho bài viết để lọc nhanh và hiển thị dữ liệu dễ dàng.</p>
      </div>
    ...
    <div class="tag-layout">
          ...
          <div v-for="tag in filteredTags" :key="tag.id" class="tag-table__row" :class="{ 'is-active': editingTagId === tag.id }">
            ...
            <div class="cell text-right">
              <div class="table-actions">
                <BaseButton size="sm" variant="ghost" @click="startEdit(tag)">Chỉnh sửa</BaseButton>
                <BaseButton
                  size="sm"
                  variant="danger"
      ...
      <div class="tag-panel tag-panel--form">
        ...
        <p class="muted-text">
          {{
            editingTagId
              ? 'Cập nhật tên hoặc slug cho thẻ đang chỉnh.'
              : 'Tạo thẻ mới để gắn vào bài viết.'
          }}
        </p>
        <form class="tag-form" @submit.prevent="handleSubmit">
          <div class="form-field">
            <label for="tag-name">Tên thẻ</label>
            <input id="tag-name" v-model="form.name" placeholder="VD: Tin tức" />
          </div>
          <div class="form-field">
